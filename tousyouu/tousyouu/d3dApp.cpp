...
bool D3DApp::Init()
{
    ...
    if (!LImGui::Initialize(MainWnd(), m_pd3dDevice.Get(), m_pd3dImmediateContext.Get()))
        return false;
...
bool D3DApp::InitDirect3D()
{
    ...
    for (UINT driverTypeIndex = 0; driverTypeIndex < numDriverTypes; driverTypeIndex++)
    {
        ...
        if (hr == E_INVALIDARG)
        {
            // Direct3D 11.0 APIはD3D_FEATURE_LEVEL_11_1を認識しないため、機能レベル11.0以下を試す必要があります
    ...
    // 機能レベル11.0または11.1をサポートしているかどうかを確認
    ...
    ComPtr<IDXGIFactory2> dxgiFactory2 = nullptr;	// D3D11.1（DXGI1.2を含む）特有のインターフェースクラス

    // DXGI SwapChainを正しく作成するためには、まずD3Dデバイスを作成したDXGIファクトリを取得する必要があります。そうしないと、以下のエラーが発生します：
    // "IDXGIFactory::CreateSwapChain: This function is being called with a device from a different IDXGIFactory."
    HR(m_pd3dDevice.As(&dxgiDevice));
    HR(dxgiDevice->GetAdapter(dxgiAdapter.GetAddressOf()));
    HR(dxgiAdapter->GetParent(IID_PPV_ARGS(dxgiFactory1.GetAddressOf())));

    // このオブジェクトがIDXGIFactory2インターフェースを含んでいるかどうかを確認
    hr = dxgiFactory1.As(&dxgiFactory2);
    // IDXGIFactory2が含まれている場合は、D3D11.1をサポートしていることを意味します
