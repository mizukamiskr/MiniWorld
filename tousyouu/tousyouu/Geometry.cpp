...
namespace Geometry
{
    ...
    GeometryData CreateBox(float width, float height, float depth)
    {
        ...
        //方向指定
        const int RIGHT_FACE_ID = 0;
        const int LEFT_FACE_ID = 1;
        const int TOP_FACE_ID = 2;
        const int BOTTOM_FACE_ID = 3;
        const int BACK_FACE_ID = 4;
        const int FRONT_FACE_ID = 5;
        ...
        geoData.faceIDs.resize(24);

        // 面を示すの変数
        for (size_t i = 0; i < 4; ++i)
        {
            geoData.faceIDs[i] = RIGHT_FACE_ID;       // 右
            geoData.faceIDs[i + 4] = LEFT_FACE_ID;    // 左
            geoData.faceIDs[i + 8] = TOP_FACE_ID;     // 上
            geoData.faceIDs[i + 12] = BOTTOM_FACE_ID; // 下
            geoData.faceIDs[i + 16] = BACK_FACE_ID;   // 後
            geoData.faceIDs[i + 20] = FRONT_FACE_ID;  // 正面
        }
