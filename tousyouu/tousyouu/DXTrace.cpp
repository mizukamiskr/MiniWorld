    ...
    _In_opt_ const WCHAR* strMsg, _In_ bool bPopMsgBox)
{
    ...
    // Windows SDK 8.0以降、DirectXのエラーメッセージはエラーコードに統合されており、FormatMessageWでエラーメッセージ文字列を取得できます。
    // 文字列メモリの割り当ては不要です。
    ...
    if (errorStr)
    {
        errorStr[0] = L'\0';	// FormatMessageWが追加する改行コードを消去します（\r\nの\rを\0に置き換えるだけでよい）。
    }

    swprintf_s(strBufferHR, 40, L" (0x%0.8x)", hr);
    wcscat_s(strBufferError, strBufferHR);
    swprintf_s(strBuffer, 3000, L"エラーコード：%ls", strBufferError);
    OutputDebugStringW(strBuffer);
