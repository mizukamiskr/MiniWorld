...
void Model::CreateFromGeometry(Model& model, ID3D11Device* device, const GeometryData& data, bool isDynamic)
{
    ...
    model.meshdatas[0].m_FaceID = (uint32_t)data.faceIDs.size();
    ...
    model.boundingbox = ComputeAABB(data.vertices);
    ...
    if (!data.faceIDs.empty()) {
        initData.pSysMem = data.faceIDs.data();
        bufferDesc.ByteWidth = (uint32_t)data.faceIDs.size() * sizeof(uint32_t);
        device->CreateBuffer(&bufferDesc, &initData, model.meshdatas[0].m_pFaceID.GetAddressOf());
    }
...
BoundingBox Model::ComputeAABB(const std::vector<XMFLOAT3>& vertices)
{
    XMVECTOR minPoint = XMLoadFloat3(&vertices[0]);
    ...
    BoundingBox aabb;
    XMStoreFloat3(&aabb.Center, (minPoint + maxPoint) * 0.5f);
    XMStoreFloat3(&aabb.Extents, (maxPoint - minPoint) * 0.5f);
