...
//座標のところで当たり判定ボックスを更新
DirectX::BoundingBox GameObject::GetBoundingBoxAtPosition(const DirectX::XMFLOAT3& position) const
{
   /* if (!m_pModel)
        return DirectX::BoundingBox(DirectX::XMFLOAT3(), DirectX::XMFLOAT3());
    ...
    DirectX::XMMATRIX originalTransformWithoutTranslation = m_Transform.GetLocalToWorldMatrixXM();
    originalTransformWithoutTranslation.r[3] = DirectX::XMVectorSet(0, 0, 0, 1);
    DirectX::XMMATRIX combinedTransform = originalTransformWithoutTranslation * translationMatrix;
    box.Transform(box, combinedTransform);
    ...
    BoundingBox box = m_pModel->boundingbox;
    DirectX::XMMATRIX translationMatrix = DirectX::XMMatrixTranslation(position.x, position.y, position.z);
    box.Transform(box, translationMatrix);
