...
struct EffectPass : public IEffectPass
{
    ...
    // シェーダーのフォーマルパラメータとしての定数バッファデータ（定数バッファは作成しない）
...
public:
    ...
    // すべてのリソースとリフレクション情報をクリアする
    void Clear();
    // Blobからシェーダーを作成し、識別名を指定する
...
public:
    ...
    std::unordered_map<size_t, std::shared_ptr<ConstantBufferVariable>> m_ConstantBufferVariables;  // 定数バッファの変数
    std::unordered_map<uint32_t, CBufferData> m_CBuffers;											    // 定数バッファの一時的なキャッシュデータ
    ...
    bool m_ForceWrite = false;      // 強制的にコンパイル後キャッシュする
};
